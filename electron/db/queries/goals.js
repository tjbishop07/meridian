...
export function getAllGoals(includeCompleted = false) {
  ...
  `;
    ...
    const goals = db.prepare(sql).all();
...
export function createGoal(data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `);
    const result = stmt.run(data.name, data.target_amount, data.current_amount || 0, data.target_date || null, data.category_id || null, data.notes || null);
    const goal = getGoalById(result.lastInsertRowid);
    if (!goal) {
...
export function addContribution(goalId, amount, date, notes) {
    const db = getDatabase();
