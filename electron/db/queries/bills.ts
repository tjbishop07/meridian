...
export function getAllBills(activeOnly: boolean = true): Bill[] {
  ...
  const bills = db.prepare(sql).all() as Bill[];
...
function calculateNextDueDate(bill: Bill): { next_due_date: string; days_until_due: number } {
...
export function getUpcomingBills(days: number = 30): Bill[] {
...
export function createBill(data: BillInput): Bill {
  const db = getDatabase();
  ...
  const bill = getBillById(result.lastInsertRowid as number);
  if (!bill) {
...
export function updateBill(data: Partial<BillInput> & { id: number }): Bill {
  const db = getDatabase();
...
export function getBillPayments(billId: number): BillPayment[] {
  const db = getDatabase();
  return db
    .prepare('SELECT * FROM bill_payments WHERE bill_id = ? ORDER BY date DESC')
    .all(billId) as BillPayment[];
}
