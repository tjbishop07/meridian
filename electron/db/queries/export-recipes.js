...
export const exportRecipeQueries = {
    getAll() {
        ...
        return db.prepare('SELECT * FROM export_recipes ORDER BY name').all();
    },
    getById(id) {
        const db = getDatabase();
        return db.prepare('SELECT * FROM export_recipes WHERE id = ?').get(id);
    },
    create(input) {
        const db = getDatabase();
        const stepsJson = JSON.stringify(input.steps);
        const result = db
            .prepare('INSERT INTO export_recipes (name, url, institution, steps, account_id) VALUES (?, ?, ?, ?, ?)')
