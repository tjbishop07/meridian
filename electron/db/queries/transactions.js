...
export function createTransaction(data) {
    ...
    if (data.type === 'transfer') {
      ...
      `);
            const result1 = stmt1.run(data.account_id, data.date, data.description, data.original_description || null, data.amount, data.balance || null, 'transfer', data.status || 'cleared', data.notes || null);
            const outgoingId = result1.lastInsertRowid;
      ...
      `);
            const result2 = stmt2.run(data.to_account_id, data.date, linkedDescription, data.original_description || null, data.amount, data.balance || null, 'transfer', data.status || 'cleared', data.notes || null, outgoingId);
            const incomingId = result2.lastInsertRowid;
    ...
    const duplicates = findDuplicateTransactions(data.account_id, data.date, data.amount, data.description);
  ...
  `);
    const result = stmt.run(data.account_id, data.category_id || null, data.date, data.description, data.original_description || null, data.amount, data.balance || null, data.type, data.status || 'cleared', data.notes || null);
...
export function updateTransaction(data) {
    ...
    if (data.type === 'transfer' && currentTransaction.type !== 'transfer') {
        ...
        return db.transaction(() => {
            ...
            const fields = ['type = ?', 'category_id = NULL'];
      ...
      `);
            const result = stmt.run(data.to_account_id, data.date || currentTransaction.date, linkedDescription, currentTransaction.original_description, data.amount || currentTransaction.amount, 'transfer', data.status || currentTransaction.status, data.notes !== undefined ? data.notes : currentTransaction.notes, data.id);
            const linkedId = result.lastInsertRowid;
            // Link them together
            db.prepare('UPDATE transactions SET linked_transaction_id = ? WHERE id = ?')
    ...
    if (data.type && data.type !== 'transfer' && currentTransaction.type === 'transfer') {
        return db.transaction(() => {
            ...
            const fields = ['type = ?', 'linked_transaction_id = NULL'];
...
export function bulkCreateTransactions(transactions) {
    const db = getDatabase();
    const stmt = db.prepare(`
    INSERT INTO transactions (
      account_id, category_id, date, description, original_description,
  ...
  `);
    ...
    `);
        for (const txn of txns) {
            // Check for duplicates before inserting
            const duplicates = findDuplicateTransactions(txn.account_id, txn.date, txn.amount, txn.description);
            if (duplicates.length > 0) {
                ...
                const needsUpdate = (txn.category_id && !existing.category_id) || // Category added
                ...
                if (needsUpdate) {
                    ...
                    updateStmt.run(txn.category_id || existing.category_id || null, txn.amount, txn.balance || existing.balance || null, txn.date, txn.description, txn.status || 'cleared', existing.id);
                    updated++;
            ...
            try {
                stmt.run(txn.account_id, txn.category_id || null, txn.date, txn.description, txn.original_description || null, txn.amount, txn.balance || null, txn.type, txn.status || 'cleared', txn.notes || null);
                count++;
            }
            catch (error) {
...
export function findDuplicateTransactions(accountId, date, amount, description) {
    const db = getDatabase();
  ...
  `).all(accountId, date, amount, description);
    if (exact.length > 0) {
  ...
  `).all(accountId, amount, amountTolerance, date, date);
