...
window.addEventListener('DOMContentLoaded', () => {
  ...
  `;
    ...
    // Update URL display
    const urlInput = document.getElementById('recording-url');
    const updateUrl = () => {
        if (urlInput) {
    ...
    saveModalSave?.addEventListener('click', async () => {
        const nameInput = document.getElementById('recording-name');
        const institutionInput = document.getElementById('recording-institution');
        ...
        try {
            await window.electron.saveRecording({ name, institution });
            saveModal?.classList.remove('visible');
        }
        catch (error) {
...
contextBridge.exposeInMainWorld('electron', {
    ...
    saveRecording: (data) => ipcRenderer.invoke('automation:save-recording', data)
});
