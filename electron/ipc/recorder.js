...
export function registerRecorderHandlers() {
    ...
    ipcMain.handle('browser:attach', async (event, url) => {
        try {
            ...
            browserView._resizeHandler = resizeHandler;
    ...
    ipcMain.handle('browser:detach', async (event) => {
        try {
            ...
            if (browserView && !browserView.webContents.isDestroyed()) {
                ...
                const resizeHandler = browserView._resizeHandler;
                if (resizeHandler) {
                    mainWindow.removeListener('resize', resizeHandler);
                    delete browserView._resizeHandler;
                }
    ...
    ipcMain.handle('browser:start-recording', async () => {
        try {
                ...
                if (message.startsWith('debug:evt:') && message.includes('{')) {
                    try {
                        ...
                        if (currentRecording) {
                            ...
                            const isAppUI = interaction.selector?.includes('e.g., Download') ||
            ...
            browserView._recorderListener = consoleListener;
            ...
            browserView._recorderDidFinishLoadListener = didFinishLoadListener;
            browserView._recorderDidNavigateListener = didNavigateListener;
    ...
    ipcMain.handle('browser:execute-step', async (_, step) => {
        try {
            if (!browserView || browserView.webContents.isDestroyed()) {
                return { success: false, error: 'Browser view not available' };
            }
    ...
    ipcMain.handle('browser:prompt-sensitive-input', async (_, label, stepNumber, totalSteps) => {
        try {
            if (!browserView || browserView.webContents.isDestroyed()) {
      ...
      `;
            ...
            console.log('[Browser] Received sensitive input, length:', value.length);
            return value;
        }
        catch (error) {
    ...
    ipcMain.handle('browser:stop-recording', async () => {
        try {
            ...
            const listener = browserView._recorderListener;
            if (listener) {
                browserView.webContents.removeListener('console-message', listener);
                delete browserView._recorderListener;
            }
            // Remove navigation listeners
            const didFinishLoadListener = browserView._recorderDidFinishLoadListener;
            if (didFinishLoadListener) {
                browserView.webContents.removeListener('did-finish-load', didFinishLoadListener);
                delete browserView._recorderDidFinishLoadListener;
            }
            const didNavigateListener = browserView._recorderDidNavigateListener;
            if (didNavigateListener) {
                browserView.webContents.removeListener('did-navigate-in-page', didNavigateListener);
                delete browserView._recorderDidNavigateListener;
            }
