...
export function registerImportHandlers() {
    // Detect CSV format
    ipcMain.handle('import:detect-format', async (_, filePath) => {
        try {
    ...
    ipcMain.handle('import:preview', async (_, data) => {
        try {
    ...
    // Execute import
    ipcMain.handle('import:execute', async (_, data) => {
        try {
            ...
            if (data.skipDuplicates) {
                ...
                console.log(`Skipping ${duplicates.length} duplicates, importing ${rowsToImport.length} rows`);
            }
            // Map categories
            const categories = db
            ...
            const incomeCategory = categories.find(c => c.type === 'income' && (c.name.toLowerCase() === 'income' || c.name.toLowerCase().includes('income')));
            ...
            const insertMany = db.transaction((rows) => {
                ...
                for (const row of rows) {
                    try {
                        ...
                        const result = insertStmt.run(data.accountId, categoryId, row.date, row.description, row.original_description, absAmount, row.balance || null, type, row.status || 'cleared');
                        if (result.changes > 0) {
                            imported++;
                        }
                        else {
        ...
        `);
            const historyResult = historyStmt.run(data.filename, data.accountId, data.format, imported, skipped, dateRange.start, dateRange.end);
            const result = {
                imported,
                skipped,
                errors: 0,
                history_id: historyResult.lastInsertRowid,
            };
