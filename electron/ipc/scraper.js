...
export function registerScraperHandlers() {
    ...
    ipcMain.handle('scraper:open-browser', async (_, options) => {
        try {
            ...
            scraperWindow.webContents.on('dom-ready', async () => {
                const hasApi = await scraperWindow.webContents.executeJavaScript('typeof window.scraper !== "undefined"');
...
async function injectScraperControls(window) {
