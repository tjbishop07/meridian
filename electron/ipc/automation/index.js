...
export function registerAutomationHandlers() {
    ...
    ipcMain.handle('automation:play-recording', async (_, recipeId) => {
    ...
    ipcMain.handle('schedule:update', async (_, { enabled, interval }) => {
 ...
 */
export async function playRecording(recipeId) {
    try {
 ...
 */
async function injectPlaybackOverlay(window, currentStep, totalSteps) {
 ...
 */
async function updatePlaybackProgress(window, currentStep, totalSteps, statusText, statusColor) {
    if (!window || window.isDestroyed())
        return;
 ...
 */
async function showStepError(window, stepIndex, totalSteps, step, error) {
    if (!window || window.isDestroyed())
        return;
    try {
        await window.webContents.executeJavaScript(`
