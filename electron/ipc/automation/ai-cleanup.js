 ...
 */
export async function cleanTransactionsWithAI(transactions, statusCallback) {
...
async function selectBestModel() {
    ...
    try {
        ...
        const foundModel = preferredModels.find((m) => availableModels.some((a) => a.startsWith(m.split(':')[0])));
        if (foundModel) {
            return foundModel;
        }
 ...
 */
function parseAIResponse(aiResponse, originalBatch, batchStartIndex) {
    try {
