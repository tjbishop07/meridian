 ...
 */
export async function scrapeWithVision(window, config) {
 ...
 */
async function captureFullPage(window) {
    const screenshots = [];
    // Get page dimensions
  ...
  `);
    ...
    if (isScrollable) {
        ...
        const numScreenshots = Math.min(Math.ceil((totalHeight - viewportHeight) / scrollStep) + 1, 6 // Cap at 6 screenshots to avoid too many API calls
        );
 ...
 */
async function extractTransactionsWithClaude(screenshots, config) {
    ...
    try {
        ...
        const scrapedTransactions = transactions.map((txn, index) => {
 ...
 */
async function extractTransactionsWithOllama(screenshots, config) {
 ...
 */
async function handlePagination(window) {
