...
import { extractIdentification, generateElementFinderScript, type ElementIdentification } from './element-finder';
 ...
 */
export async function executeStep(window: BrowserWindow, step: RecordingStep): Promise<void> {
 ...
 */
async function executeStepInternal(window: BrowserWindow, step: RecordingStep): Promise<void> {
 ...
 */
async function executeClickStep(window: BrowserWindow, identification: ElementIdentification): Promise<void> {
 ...
 */
async function executeInputWithCoordinates(
  window: BrowserWindow,
  identification: ElementIdentification,
  value?: string
): Promise<void> {
 ...
 */
async function executeInputStep(window: BrowserWindow, identification: ElementIdentification, value?: string): Promise<void> {
 ...
 */
async function executeSelectStep(window: BrowserWindow, identification: ElementIdentification, value?: string): Promise<void> {
 ...
 */
async function checkPageResponsive(window: BrowserWindow): Promise<boolean> {
  if (!window || window.isDestroyed()) {
    return false;
  }
 ...
 */
async function waitForPageLoad(window: BrowserWindow, timeout: number = 5000): Promise<void> {
  if (!window || window.isDestroyed()) {
    return;
  }
 ...
 */
async function getPageDiagnostics(window: BrowserWindow): Promise<string> {
  try {
